...
\centering\vspace*{-.25em}\begin{tikzpicture}[bc/.default={lgray},br/.default={lgray}]
    ...
    \begin{scope}[xshift=6.15cm,yshift=.75cm]
        ...
        \foreach \i/\a in {0/7,1/6,2/5,3/4} {%
            \def\c{lgray}%
        ...
        \begin{pgfonlayer}{background}
            \draw[tarrow=lgray,line cap=round] (t3)++(0,-.5cm) coordinate (lrc-end) -- ($(t0)+(0,0.75cm)$);
            \draw[t=lgray,line cap=round] (tc1) -- (lrc-end-|tc3) (t0) -- (tc1) -- (t1) (t2) -- (tc3) -- (t3);
        \end{pgfonlayer}
    ...
    \begin{pgfinterruptboundingbox}
        \fill[lgray,rounded corners=1.5pt] (-1.66-.45,0) rectangle (9.5+.75+.25,0.1);
        \draw[line width=.5ex,btdl@color@background] (1.66,0) -- ++(0,.1);
        \draw[line width=.5ex,btdl@color@background] (5,0) -- ++(0,.1);
        \draw[line width=.5ex,btdl@color@background] (8,0) -- ++(0,.1);
    \end{pgfinterruptboundingbox}
    \end{scope}
...
\begin{frame}{Conclusion}
    [TODO] Rückblick: Problem. Dann Übersicht: DDC + CHAINIAC + weitere Probleme: eine Option in vielen. => Rückblende zu warum \enquote{Trust}
\end{frame}
